...
6.2.3 temai „Dalijimas proporcingai“ ir įterpia HINTS + VARIANTS iš _data.js."""
import re, io
...
html = html.replace(
    ...
    "<title>Pabėgimas · Dalijimas proporcingai · 6 klasė</title>")

# 2. badge
html = html.replace("Pabėgimo planas · Kamera Nr. 6", "Slaptoji operacija · Seifas Nr. 235")
...
html = html.replace(
    ...
    'Pabėgimo kambarys · Dalijimas proporcingai · 6 klasė')
...
new_story = '''<div class="story">
      ...
      <p style="margin:12px 0 0"><i>„Kiekvienas seifo ratukas atsirakina teisingai padalijus dydį nurodytu santykiu.
      Kiekvienas ratukas duoda po vieną seifo kodo skaitmenį (vienaženklę dalį). Surink visą kodą, atidaryk
      seifą ir dink.“</i></p>
...
html = html.replace("<b>🔒 10 grandinių.</b> Atrakinamos iš eilės, sprendžiant skliaustų lygtis.",
                    "<b>🔒 10 ratukų.</b> Atrakinami iš eilės dalijant dydį nurodytu santykiu.")
...
html = html.replace(old_wintext,
    ...
    'Dalyti dydžius nurodytu santykiu tau sekasi geriau nei bet kuriam saugyklos sargui.</p>')

# rangai (JS eilutėse)
html = html.replace('"🥇 Legendinis bėglys (be patarimų ir klaidų!)"',
                    '"🥇 Legendinis agentas (be patarimų ir klaidų!)"')
html = html.replace('"🥈 Patyręs bėglys"', '"🥈 Patyręs agentas"')
html = html.replace('"🥉 Vos spėjai — bet pabėgai!"', '"🥉 Vos spėjai, bet pabėgai!"')
