...
6.1.3 temai („Ieškome viso skaičiaus") ir įterpia sugeneruotus HINTS + VARIANTS iš _data.js."""
import re, io
...
html = html.replace(
    ...
    "<title>Pabėgimas · Ieškome viso skaičiaus · 6 klasė</title>")
...
html = html.replace(
    ...
    'Pabėgimo kambarys · Ieškome viso skaičiaus · 6 klasė')
...
new_story = '''<div class="story">
      ...
      <p style="margin:12px 0 0"><i>„Kiekvienas seifo ratukas atsirakina, kai iš žinomos dydžio dalies (kiek procentų ji
      sudaro) atkuri visą skaičių. Kiekvienas ratukas duoda po vieną seifo kodo skaitmenį. Surink visą kodą, atidaryk
      seifą ir dink.“</i></p>
...
html = html.replace("<b>🔒 10 grandinių.</b> Atrakinamos iš eilės, sprendžiant skliaustų lygtis.",
                    "<b>🔒 10 ratukų.</b> Atrakinami iš eilės, iš žinomos dalies atkuriant visą skaičių.")
...
# 7. įvedimo etiketė („x =" -> „Skaitmuo:")
html = html.replace('<span class="lead">x =</span>', '<span class="lead">Skaitmuo:</span>')
...
html = html.replace(old_wintext,
    ...
    'Iš bet kokios dalies akimirksniu atkuri visą skaičių, geriau nei bet kuris saugyklos sargas.</p>')
...
# 10. įterpiam duomenis: pakeičiam HINTS ... VARIANTS ]; bloką
